...
var DES3 = {
    //使用PKCS7 padding
    encrypt:function(input,encryptKey,iv){
        var result = '';
        var genKey = genkey(encryptKey, 0, 24);
        result = btoa(des(genKey.key, input, 1, 1, iv, 1));
        return result;
    },
    //使用PKCS7 padding
    decrypt:function(input,encryptKey,iv){
        var genKey = genkey(encryptKey, 0, 24);
        return des(genKey.key, atob(input), 0, 1, iv, 1);
    }
};

function doEncrytion(obj,$encryptKey,$iv){
    return DES3.encrypt(obj,$encryptKey,$iv);
}